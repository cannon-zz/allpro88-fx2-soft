...
def blink_zif_pin1():
	...
	for pin in range(88):
		programmer.write_command("=", programmer.pin_addr(pin), 0x01)
	# set pin 40 DAC to 35 (3 V)
	pin = 40
	programmer.write_command("=", programmer.pin_addr(pin) + 3, 255)
	programmer.write_command("=", 0x0308, 0)
	# toggle pin 40 (zif socket pin 1) between VTST and ground
	for i in tqdm(range(10), desc = "blink BUSY LED"):
		#programmer.write_command("=", programmer.pin_addr(pin), 0x04)
		programmer.write_command("=", programmer.pin_addr(pin), 0x10)
		time.sleep(0.5)
		programmer.write_command("=", programmer.pin_addr(pin), 0x01)
		time.sleep(0.5)
	# set all pins to disable
	for pin in range(88):
		programmer.write_command("=", programmer.pin_addr(pin), 0)
