...
class allpro88(object):
	...
	def measure_vadj(self):
		...
		power supply's output voltage.  NOTE:  VADJTH is left set
		to (an approximation of) the measured voltage.
		"""
		vdac, = self.write_command("V")

		# both VADJ and VADJTH are generated by U3 on analogue
		# control 1, an AD7226.  this chip's voltage output is Vref
		# * (DAC value) / 256 (note:  max voltage is 1 LSB less
		# than Vref).  Vref for the chip is generated by an LM317
		# with an adjustment crazy-glued in place.  this is an
		# important calibration point in the circuit.  in mine, the
		# Vref is +5.06 V, but I believe it's meant to be 5.1 V.
		# since mine is out by less than 1% I'm going to call that
		# OK.

		# convert numeric DAC value to DAC output voltage
		vdac *= 5.1 / 256.

		# the adjustable power supply feed-back loop compares VADJ
		# to the DAC output voltage via a resistor divider
		# consisting of a 5.1 kOhm and 1 kOhm set of resistors.
		# the regulator ensures that VADJ * 1 kOhm / (5.1 kOhm + 1
		# kOhm) = DAC voltage

		# multiply the DAC output voltage by resistor divider ratio
		# to get the expected value of VADJ
		# NOTE:  altogether, the relationship between VADJ output
		# and the number put into the DAC should be VADJ = dac *
		# 0.1215234375.  this is close to, but not quite the
		# formula in kevtris' notes.  he reports a smaller slope, a
		# positive voltage offset at 0, and a noticable quadratic
		# term.  I've only bothered to confirm that my adjustable
		# power supply produces voltages spanning approximately the
		# correct range, I haven't bothered, yet, checking the
		# functional relationship between DAC value and voltage,
		# and it might be worth doing it.
		return vdac * ((5.1 + 1.) / 1.)
